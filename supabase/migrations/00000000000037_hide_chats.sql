-- Per-user conversation hiding (swipe to delete).
-- Hiding is scoped to the acting user; the other DM participant is
-- unaffected. A hidden chat reappears if a newer message arrives.

alter table public.chat_members
  add column if not exists hidden_at timestamptz;

create or replace function public.hide_chat(p_chat_id uuid)
returns void
language sql
...
create or replace function public.unhide_chat(p_chat_id uuid)
returns void
language sql
security definer set search_path = public
as $$
...
grant execute on function public.unhide_chat(uuid) to authenticated;
