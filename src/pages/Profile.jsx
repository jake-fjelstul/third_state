    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:12, padding:'0 20px 28px', justifyContent:'center' }}>
          {[
            { value: currentUser.stats?.circlesJoined  ?? joinedCircles.length, label:'CIRCLES'     },
            { value: currentUser.stats?.meetupsAttended ?? meetups.length,       label:'MEETUPS'     },
            { value: currentUser.stats?.connections     ?? 85,                   label:'CONNECTIONS' },
          ].map(({ value, label }) => (
            <div key={label} style={{
              flex: 1,
        ...
        <div style={{ padding:'0 20px 28px' }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            <button type="button" style={{ background:'none', border:'none', cursor:'pointer', fontSize:14, fontWeight:600, color: clr.indigo }}>
              See All
            </button>
          </div>
